...
def two(input):
    noCase = input.lower()
    if noCase.count("bert") < 2:
        return ""
    else:
        print(noCase)
        return noCase.strip("bert")
...
def four(arg1):
    ...
    for n in lst:
        if index < len(lst):
            if n.isdigit() and lst[index].isdigit():
                added = int(n) + int(lst[index])
                numberList.append(added)
            index +=1
...
def six(input):
    index = 0
    for i in input:
        if i == "c" and input[index +1] == "i" and input[index +2] == "e":
            return False
        elif  i =="c" and input[index +1] == "e" and input[index +2] == "i":
            return True  
        elif i == "i" and input[index + 1] == "e":
            return True
        elif i == "e" and input[index +1] == "i":
            return False
        index += 1
...
def ten(string, int, char):
    ...
    if len(noSpaceWord) < int:
        return False
    elif noSpaceWord[int - 1] == char:
        return True
    else:
        return False